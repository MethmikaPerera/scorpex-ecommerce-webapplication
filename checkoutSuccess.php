...
if (isset($_SESSION['u'])) {
    ...
    if (isset($_SESSION['cart'])) {
        ...
        $order_total = $session_cart['cart_total'];
        $orderid = $session_cart['orderid'];
        $address_id = $session_cart['selected_address'];
        ...
        for ($i = 0; $i < $cart_num; $i++) {
            ...
            $product_sizeid = $cart_data['size_id'];

            Database::iud("INSERT INTO `orders`(`order_id`,`date`,`total`,`qty`,`size_id`,`product_id`,`user_id`,`order_status`,`address_id`) 
            VALUES ('" . $orderid . "','" . $date . "','" . $order_total . "','" . $product_qty . "','" . $product_sizeid . "','" . $product_id . "','" . $userid . "','1','2')");
